...
export default function CompetitionLivePage({
  ...
  const fetchData = async () => {
    try {
      ...
      try {
        ...
        // competition_groups table may not exist, continue with default group
      }

      // Fetch group assignments with students (may not exist)
      try {
        const result = await supabase
        ...
        // group_assignments table may not exist, continue with default group
      }
  ...
  const fetchRoundStudents = async () => {
    try {
      ...
      // Get group assignments to organize students by groups (may not exist)
      let assignmentsData = null;
      try {
        const result = await supabase
          .from("group_assignments")
  ...
  const handleTimeEntry = async (studentId: string) => {
    ...
    try {
      ...
      // Save to database using upsert to handle existing attempts
      ...
      // Refresh student progress from database to show updated results
