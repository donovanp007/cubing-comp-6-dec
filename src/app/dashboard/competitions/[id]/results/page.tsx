...
import { Trophy, Medal, Users, Zap, Heart, ArrowLeft, Download } from "lucide-react";
...
export default function CompetitionResultsDashboard({
  params,
}: {
  ...
  const downloadResults = () => {
    // Generate CSV data
    const headers = ["Place", "Name", "Grade", "Best Time", "Average", "Status"];
    const rows = roundResults.map((r, i) => [
      i + 1,
      `${r.first_name} ${r.last_name}`,
      r.grade,
      r.best_time_milliseconds ? formatTime(r.best_time_milliseconds) : "-",
      r.average_time_milliseconds ? formatTime(r.average_time_milliseconds) : "-",
      r.advancement_status,
    ]);
    ...
    link.href = url;
    link.download = `${competition?.name || "results"}-round-${selectedRound}.csv`;
    link.click();
    window.URL.revokeObjectURL(url);

    toast({
      title: "Downloaded",
      description: "Results CSV exported successfully",
    });
  };
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto">
      {/* Header */}
      <div className="mb-8">
        <Link href={`/dashboard/competitions/${competitionId}`} className="text-sm text-gray-500 hover:text-gray-700 flex items-center gap-1 mb-4">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-500 mt-2">Final Results & Standings</p>
          </div>
        ...
        <Card>
          ...
          <CardContent>
            <Tabs value={selectedRound} onValueChange={setSelectedRound}>
              <TabsList className="grid w-full" style={{ gridTemplateColumns: `repeat(${Math.min(rounds.length, 5)}, 1fr)` }}>
                {rounds.map((round) => (
